...
def tag_one(
    img: Union[str, Image.Image], thr: float, model: str, replace: bool = False
) -> dict:
...
def main():
    ...
    parser.add_argument(
        "--model", default="SwinV2", choices=list(tagger_model_names.keys())
    )
    parser.add_argument(
        "--thr", type=float, default=0.4, help="probability threshold (default 0.4)"
    )
